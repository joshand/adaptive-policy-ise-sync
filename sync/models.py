    ...
    def get_data(self, attrname):
        if self.source_data:
            if attrname == "cleaned_name" or attrname == "cleaned_desc":
                ...
                for l in lacl:
                    if self.iseserver:
                        acld = ACLData.objects.filter(source_id=l).filter(iseserver=self.iseserver).first()
                    else:
                        acld = ACLData.objects.filter(source_id=l).filter(organization=self.organization).first()
...
class DataPipeline(models.Model):
    ...
    def __str__(self):
        if self.iseserver:
            ...
            return "ELEMENT=" + str(self.element) + "::" + str(self.stage) + "::" + str(self.state)
                ...
                if d == o.element:
                    ...
                    if not gen_match:
                        ...
                        if api_template:
                            ...
                            if not success:
                                if detail:
                                    outs.append(("ERROR", None, err, None, None, None, None, self, None))
                                else:
