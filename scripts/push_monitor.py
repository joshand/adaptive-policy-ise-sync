from sync.models import SyncSession, DataPipeline, TagData, ACLData, PolicyData, Tag, ACL, Policy, ElementSync,\
    GenericData, Generic, APICallTemplate
...
def make_api_call(api_call_details, log):
    ...
    if req.ok and second_run:
        if req.status_code == 201:
            ...
            new_url = url
        _, d_out, _, _, _ = make_api_call(("GET", new_url, None, auth, headers, None, None, generic, element), log)
...
def run_push_check():
    ...
    for sync in syncs:
        ...
        if sync.enabled:
            ...
            append_log(log, mod_name + "::" + fn_name + "::Needed updates:", needs_update, updates)
